...
class LogType {
     ...
     */
    static final LogType SIGN = new LogType(
            "/logs/signs.txt",
            "sign-log",
            LogListener.SignListener::new
    );

    /**
     * Log type for when a player sends a direct message to another player via Strings /msg.
     */
    static final LogType DIRECT_MESSAGE = new LogType(
            "/logs/direct-messages.txt",
            "dm-log",
            LogListener.DirectMessageListener::new
    );

    /**
     * Log type for all chat messages sent in Channels that call an event.
     */
    static final LogType CHAT = new LogType(
            "/logs/chat.txt",
            "chat-log",
            LogListener.ChatListener::new
    );

    /**
     * Log type sent when players send commands.
     */
    static final LogType COMMAND = new LogType(
            "/logs/commands.txt",
            "command-log",
            LogListener.CommandListener::new
    );

    /**
     * Log type sent when a chat message is filtered.
     */
    static final LogType FILTER = new LogType(
            "/logs/filtered-messages.txt",
            "filter-log",
            LogListener.ChatFilterListener::new
    );
